...
bool implicitly_cast(Reporter *reporter, AstExpression **_expression, AstExpression *type) {
	...
	if (expression->kind == Ast_literal) {
		...
		//
		// TODO: check for casts that should not happen
		//
		auto cast = new_ast<AstCast>();
		cast->type = type;
		cast->expression = ((AstAutocast *)expression)->expression;
		expression = cast;
		return true;
...
restart_main:
	...
	init_type(type_autocast, u8"autocast"s, 0, 0);
