...
void output_fasm_x86_64_windows(Bytecode &bytecode) {
	timed_function();
...
{}\fasm\fasm.exe "{}"
	...
	auto bat_path = to_pathchars(concatenate(executable_directory, u8"\\fasm_build.bat"s));
	write_entire_file(bat_path, as_bytes(to_string(bat_builder)));

	_putenv(tformat("Include={}/fasm/include{}", executable_directory, '\0').data);
